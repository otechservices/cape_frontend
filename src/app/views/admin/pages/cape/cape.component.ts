...
export class CapeComponent implements OnInit {
  ...
  getAll(){
    this.capeService.getAll().subscribe((res:any)=>{
      if (this.role=="admin") {
        this.data=res.data.filter((el:any)=> el.is_authorized==true)
  ...
  store(value:any){
    ...
    (err:any)=>{
      ...
      console.log(err)
        AppSweetAlert.simpleAlert("error","Cape",err.error.message)
    })
  }
  update(value:any){
    this.loading=true
