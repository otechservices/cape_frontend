...
export class ActualityService {
  ...
  url=ConfigService.toApiUrl("actualities/");

  constructor(private http:HttpClient) { }

  getAll(){
    return this.http.get<any[]>(`${this.url}`, ConfigService.httpHeader(localStorage.getItem(GlobalName.tokenName),true));
  }
  
  getAll2(){
    return this.http.get<any[]>(`${ConfigService.toApiUrl("actualities-index-2")}`);
  }
